...
export async function gen_douban_book (sid) {
  ...
  if (douban_page_raw.match(/你想访问的页面不存在/)) {
    ...
    data.author = author = $('#info span.pl:contains("作者")').siblings('a').map(function () {
    ...
    const isbnFetch = await fetch(`http://api.feelyou.top/isbn/${ISBN}`, {
      headers: {
        apikey: 'OWHagO3Wmkt0FLaZJTtgHxCzGDxHt0Uu',
      },
    });
    const isbnText = await isbnFetch.text();
    data.poster = poster = JSON.parse(isbnText).cover_url;
