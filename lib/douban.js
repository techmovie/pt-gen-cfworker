...
export async function search_douban (query) {
  const douban_search = await fetch(`https://movie.douban.com/j/subject_suggest?q=${query}`);
...
export async function gen_douban (sid) {
  ...
  const db_page_resp = await fetch(douban_link); // 请求豆瓣对应项目主页面
  ...
  if (douban_page_raw.match(/你想访问的页面不存在/)) {
    ...
    const awards_page_req = fetch(`${douban_link}awards`); // 马上请求豆瓣获奖界面
    ...
    const ld_json = JSON.parse($('head > script[type="application/ld+json"]').html().replace(/(\r\n|\n|\r|\t)/gm, ''));
    ...
    // 提前imdb相关请求
    const imdb_link_anchor = $('#info span.pl:contains("IMDb")');
    const has_imdb = imdb_link_anchor.length > 0;
    if (has_imdb) {
      const imdbId = fetch_anchor(imdb_link_anchor);
      data.imdb_link = imdb_link = `https://www.imdb.com/title/${imdbId}/`;
      data.imdb_id = imdb_id = imdbId;
      const imdb_api_resp = await fetch(`https://p.media-imdb.com/static-content/documents/v1/title/${imdb_id}/ratings%3Fjsonp=imdb.rating.run:imdb.api.title.ratings/data.json`);
    ...
    const foreign_title = data.foreign_title = $('span[property="v:itemreviewed"]').text().replace(data.chinese_title, '').trim();
    ...
    const introduction_another = $('#link-report > span.all.hidden, #link-report > [property="v:summary"]');
