...
export async function gen_imdb (sid) {
  ...
  // 第二部分： Details
  const details_another = $('div#titleDetails');
  const title_anothers = details_another.find('div.txt-block');
  const details_dict = {};
  title_anothers.each(function () {
    const title_raw = $(this).text().replace(/\n/ig, ' ').replace(/See more »|Show more on {3}IMDbPro »/g, '').trim();
    if (title_raw.length > 0) {
      const title_key = title_raw.split(/: ?/, 1)[0];
  ...
  const storyLine = $('#titleStoryLine h2 + .inline.canwrap span');
