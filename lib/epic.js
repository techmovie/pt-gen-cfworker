...
export async function gen_epic (sid) {
  ...
  const epic_api_resp = await fetch(`https://store-content.ak.epicgames.com/api/zh-CN/content/products/${sid}`);
  ...
  const epic_api_json = await epic_api_resp.json();

  // 从顶层字典中获得page
  const page = epic_api_json.pages[0];
  ...
  const requirements = page.data.requirements || [];

  // 语言
  const languages = [];
  for (let i = 0; i < requirements.languages.length; i++) {
    const lang = requirements.languages[i];
