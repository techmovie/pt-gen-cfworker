...
export async function gen_steam (sid) {
  ...
  const steam_page_resp = await fetch(`https://store.steampowered.com/app/${sid}/?l=schinese`, {
    redirect: 'manual',
  ...
  // 立即请求附加资源
  const steamcn_api_req = fetch(`https://steamdb.keylol.com/app/${sid}/data.js?v=38`);
  const $ = page_parser(await steam_page_resp.text());

  // 从网页中定位数据
  const name_anchor = $('div.apphub_AppName') || $('span[itemprop="name"]'); // 游戏名
  const cover_anchor = $('img.game_header_image_full[src]'); // 游戏封面图
  const detail_anchor = $('div.details_block'); // 游戏基本信息
  const linkbar_anchor = $('a.linkbar'); // 官网
  const language_anchor = $('table.game_language_options tr[class!=unsupported]'); // 支持语言
  const tag_anchor = $('a.app_tag'); // 标签
  const rate_anchor = $('div.user_reviews_summary_row'); // 游戏评价
  const descr_anchor = $('div#game_area_description'); // 游戏简介
  const sysreq_anchor = $('div.sysreq_contents > div.game_area_sys_req'); // 系统需求
  const screenshot_anchor = $('div.screenshot_holder a'); // 游戏截图
  ...
  data.detail = detail_anchor
    ? detail_anchor.eq(0).text()
      ...
      .join('\n')
    : '';
  data.tags = tag_anchor
    ? tag_anchor.map(function () {
      return $(this).text().trim();
    }).get()
    : [];
  data.review = rate_anchor
    ? rate_anchor.map(function () {
  ...
  data.language = language_anchor
    ? language_anchor
      ...
      .map(function () {
        const tag = $(this);
        const tag_td_list = tag.find('td');
        const lag_support_checkcol = [];
        const lag = tag_td_list.eq(0).text().trim();

        for (let i = 0; i < lag_checkcol_list.length; i++) {
          const j = tag_td_list.eq(i + 1);
          if (j.text().search('✔')) {
  ...
  data.screenshot = screenshot_anchor
    ? screenshot_anchor.map(function () {
      const dic = $(this);
  ...
  data.sysreq = sysreq_anchor
    ? sysreq_anchor.map(function () {
      const tag = $(this);
      const os_type = os_dict[tag.attr('data-os')];

      const clone_tag = tag.clone();
      clone_tag.html(tag.html().replace(/<br>/ig, '[br]'));

      const sysreq_content = clone_tag
        .text()
  ...
  // 处理附加资源
  const steamcn_api_resp = await steamcn_api_req;
  const steamcn_api_jsonp = await steamcn_api_resp.text();
  const steamcn_api_json = jsonp_parser(steamcn_api_jsonp);
